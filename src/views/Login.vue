        ...
        validation-observer( ref="observer" v-slot="{ passes }" )
            b-form#login-form.w-100.mx-auto.p-5.bg-light.rounded-sm.shadow( @submit.prevent="passes(submit)" )
                ...
                validation-provider( rules="required" name="Ваш логин (e-mail)" v-slot="{ errors }")
                    b-form-group.my-3
                        b-form-input#login.border-warning( v-model="form.email" type="email" autocomplete="off" placeholder="Логин" )
                ...
                validation-provider( rules="required" name="Пароль" v-slot="{ errors }" )
                    b-form-group.my-3
                        b-input-group
                            b-form-input#password.border-warning( v-model="form.password" type="password" placeholder="Пароль")
                        transition( enter-active-class="animated fadeIn" leave-active-class="animated fadeOut" mode="out-in" )
                            div.invalid.text-danger.my-1
                                small {{ errors[0] }}
                b-form-group.my-4.text-center
                    b-button(type="submit" size="lg" variant="warning" ) Войти
</template>
